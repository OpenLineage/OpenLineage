...
// FileRecord is the structure written to disk for each OpenLineage resource.
// It wraps the raw OL event with metadata useful for auditing and debugging.
//
...
type FileRecord struct {
	...
	// Entity identifies which OpenLineage resource (job, run, or dataset) this
	// record belongs to.
	...
	// RunID is the UUID of the most recent OL RunEvent (empty for job/dataset events).
	RunID string `json:"run_id"`

	// Event is the raw OpenLineage event JSON exactly as it would be sent
...
// EntityRef is the minimal identifier embedded in FileRecord.
// Used for jobs, runs, and datasets — kept intentionally neutral.
