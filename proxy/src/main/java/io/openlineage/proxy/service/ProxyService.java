...
public final class ProxyService
{
  ...
  public ProxyService(final ProxyConfig config)
  {
    ...
    {
      this.lineageStreams = ImmutableSet.of(new ConsoleLineageStream(), new KafkaLineageStream(config.getLineageSourceName(), config.getKafkaTopicName(), config.getKafkaBootstrapServerURL(), config.getKafkaProperties()));
    }
    else
    {
      this.lineageStreams = ImmutableSet.of(new KafkaLineageStream(config.getLineageSourceName(), config.getKafkaTopicName(), config.getKafkaBootstrapServerURL(), config.getKafkaProperties()));
    }
  }
