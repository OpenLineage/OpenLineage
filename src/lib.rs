...
use sqlparser::ast::{Select, SetExpr, Query, Statement, With, TableFactor};
...
fn parse_select(select: &Select, context: &mut Context) -> Result<(), String>{
    for table in &select.from {
        match &table.relation {
            ...
            _ => return Err(String::from("TableFactor other than straight table not implemented"))
        }
...
fn parse_query(query: &Query, context: &mut Context) -> Result<(), String> {
    ...
    match &query.body {
        ...
        other => {
            println!("Other: {}", other);
            return Err(String::from("not a select"))
        }
    };
    Ok(())
}
...
fn parse_stmt(stmt: &Statement, context: &mut Context) -> Result<(), String> {
    match stmt {
        ...
        Statement::Insert{
            ...
            parse_query(source, &mut context)?;
...
fn parse_sql(sql: &str) -> Result<QueryMetadata, String> {
    ...
    parse_stmt(stmt.unwrap(), context)?;
...
mod tests {
    ...
    #[test]
    fn insert_values() {
        assert_eq!(parse_sql("INSERT INTO TEST VALUES(1)").unwrap(), QueryMetadata{
            inputs: vec![],
    ...
    fn insert_cols_values() {
        assert_eq!(parse_sql("INSERT INTO tbl(col1, col2) VALUES (1, 2), (2, 3)").unwrap(), QueryMetadata{
            inputs: vec![],
    ...
    fn insert_select_table() {
        assert_eq!(parse_sql("INSERT INTO TEST SELECT * FROM TEMP").unwrap(), QueryMetadata{
    ...
    #[test]
    fn drop_errors() {
        assert_eq!(parse_sql("DROP TABLE TEST"), Err(String::from("not a insert")));
    }

    #[test]
    fn select_simple() {
        assert_eq!(parse_sql("SELECT * FROM table0;").unwrap(), QueryMetadata{
    ...
    fn select_from_schema_table() {
        assert_eq!(parse_sql("SELECT * FROM schema0.table0;").unwrap(), QueryMetadata{
