...
class DatadogConfig(Config):
    ...
    def from_dict(cls, params: dict[str, Any]) -> DatadogConfig:
        ...
        if site not in SITE_MAPPING and not _is_valid_url(site):
...
class DatadogTransport(Transport):
    ...
    def _should_use_async_transport(self, event: Event) -> bool:
        """
        Determine if event should use async transport based on JobTypeJobFacet.
        Returns True if the event has JobTypeJobFacet with integration="dbt".
        """
        ...
        return getattr(job_type_facet, "integration", "").lower() == "dbt"
