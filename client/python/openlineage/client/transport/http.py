...
class EventTracker:
    ...
    def mark_success(self, event_id: str, run_id: str | None = None, event_type: str | None = None):
        with self.lock:
            ...
            if self.events.get(event_id) == EventStatus.PENDING:
    ...
    def mark_failed(self, event_id: str):
        with self.lock:
            log.debug("Marking event %s as failed", event_id)
            self.stats["pending"] -= 1
            self.stats["failed"] += 1
            if self.events.get(event_id) == EventStatus.PENDING:
                del self.events[event_id]
...
class AsyncEmitter(Emitter):
    ...
    async def _process_event(
        ...
        log.debug("Processing event %s", event_id)
            ...
            for attempt in range(self.config.retry.get("total", 5) + 1):
                ...
                try:
                    ...
                    if resp.status_code not in self.config.retry.get(
                        ...
                        break
                    elif attempt == self.config.retry.get("total", 5):
                        handle_failure(response=resp)
                        break
        ...
        return pending_events, event_id
