...
import pytest
from openlineage.client.transport.http import AsyncConfig, HttpConfig, HttpTransport
...
class TestBasicEventFlow:
    ...
    def test_sync_transport_single_event(self, sync_transport, server_helper):
        ...
        assert response is not None  # SyncEmitter returns httpx.Response
...
class TestAsyncEventOrdering:
    ...
    def test_multiple_completion_events(self, async_transport_custom, server_helper):
        ...
        try:
            ...
            transport.shutdown(wait=True, timeout=5.0)
...
class TestPerformanceAndLoad:
    ...
    def test_high_throughput_events(self, async_transport_custom, server_helper, num_runs, max_concurrent):
        ...
        try:
            ...
            # Debug: Print actual events received if count doesn't match
            if stats["total_events"] != expected_events:
                events = server_helper.get_events()
                print(f"\nDEBUG: Expected {expected_events} events, got {stats['total_events']}")
                print("Events received:")
                for i, event in enumerate(events):
                    print(f"  {i+1}. {event['event_id']} - {event['event_type']} - {event['run_id']}")
                print(f"Stats: {stats}")
                ...
                print(f"Events per run: {run_counts}")
                print(f"Expected 2 events per run for {num_runs} runs")
            ...
            print(
                f"Processed {expected_events} events in {processing_time:.2f}s "
                f"with {max_concurrent} concurrent"
            )
...
class TestConfiguration:
    ...
    def test_custom_endpoint(self, test_server_url, server_helper):
        ...
        config = HttpConfig(
            ...
            async_config=AsyncConfig(enabled=True),
        )
    ...
    def test_custom_headers(self, test_server_url, server_helper, custom_headers):
        ...
        config = HttpConfig(
            url=test_server_url, custom_headers=custom_headers, async_config=AsyncConfig(enabled=True)
        )
        transport = HttpTransport(config)
        ...
        try:
            ...
            # Check that custom headers were included (FastAPI lowercases headers)
    ...
    def test_different_async_configs(
        ...
        config = HttpConfig(
            ...
            async_config=AsyncConfig(
                enabled=True, max_queue_size=max_queue_size, max_concurrent_requests=max_concurrent_requests
            ),
        )
        transport = HttpTransport(config)
