...
def test_client_with_amazon_datazone_transport_emits_run_event(run_event: RunEvent, domain_id: str, mocker: MockerFixture) -> None:
...
def test_client_with_amazon_datazone_transport_skips_job_event(job_event: JobEvent, mocker: MockerFixture) -> None:
    mocker.patch("boto3.client")
    config = AmazonDataZoneConfig(
        domain_id=domain_id
    )
...
def test_client_with_amazon_datazone_transport_skips_dataset_event(dataset_event: DatasetEvent, mocker: MockerFixture) -> None:
    mocker.patch("boto3.client")
    config = AmazonDataZoneConfig(
        domain_id=domain_id
    )
