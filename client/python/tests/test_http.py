...
class TestEventTracker:
    ...
    def test_mark_success_regular_event(self):
        ...
        pending_events = tracker.mark_success("event-1")
    ...
    def test_mark_success_start_event(self):
        tracker = EventTracker()
        tracker.add_event("start-event")

        pending_events = tracker.mark_success("start-event", run_id="run-123", event_type="START")
    ...
    def test_mark_success_start_event_releases_pending(self):
        ...
        tracker.add_pending_completion_event("run-123", "complete-event", b"data", {"header": "value"})

        # Mark START as successful
        pending_events = tracker.mark_success("start-event", run_id="run-123", event_type="START")

        assert len(pending_events) == 1
        assert pending_events[0] == ("complete-event", b"data", {"header": "value"})
    ...
    def test_add_pending_completion_event(self):
        ...
        assert "run-123" in tracker.pending_completion_events
        assert len(tracker.pending_completion_events["run-123"]) == 1
        assert tracker.pending_completion_events["run-123"][0] == (
            "complete-event",
            b"data",
            {"header": "value"},
        )
    ...
    def test_get_stats(self):
        ...
        tracker.add_event("event-1")
        tracker.add_event("event-2")
        tracker.mark_success("event-1", run_id="run-123", event_type="START")
        tracker.mark_failed("event-2")
        ...
        def worker(thread_id):
            try:
                for i in range(100):
                    ...
                    tracker.add_event(event_id)
                    if i % 2 == 0:
                        tracker.mark_success(event_id)
                    else:
...
class TestAsyncEmitter:
    ...
    def test_async_emitter_event_id_generation_run_event(self, async_config):
        ...
        with patch.object(emitter.event_queue, "put") as mock_put, patch.object(
            ...
            event_id, body, headers, run_id_arg, event_type = mock_put.call_args[0][0]

            assert event_id == f"{run_id}-START"
    ...
    def test_async_emitter_event_id_generation_non_run_event(self, async_config):
        ...
        with patch("openlineage.client.serde.Serde.to_json", return_value=event_json):
            with patch.object(emitter.event_queue, "put") as mock_put:
                ...
                mock_put.assert_called_once()
                event_id, body, headers, run_id, event_type = mock_put.call_args[0][0]
    ...
    def test_async_emitter_completion_without_start_scheduled(self, async_config):
        ...
        with patch.object(emitter.event_queue, "put") as mock_put:
            ...
            mock_put.assert_called_once()
            event_id, body, headers, run_id_arg, event_type = mock_put.call_args[0][0]
            assert event_id == f"{run_id}-COMPLETE"

            # Should not be in pending events
            assert run_id not in emitter.event_tracker.pending_completion_events
    ...
    def test_async_emitter_get_stats(self, async_config):
        ...
        # Add some events
        emitter.event_tracker.add_event("event-1")
        emitter.event_tracker.add_event("event-2")
        emitter.event_tracker.mark_success("event-1")
        emitter.event_tracker.mark_failed("event-2")
