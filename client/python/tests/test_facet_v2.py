...
def test_tags_run_facet() -> None:
    # Ensure a consistent producer
    set_producer(PRODUCER)
    session = mock.MagicMock()
    client = OpenLineageClient(url="http://example.com", session=session)

    tags_run_facet = tags_run.TagsRunFacet(
        tags=[
            tags_run.TagsRunFacetFields(key="test_tag", value="test_value", source="test_source"),
            tags_run.TagsRunFacetFields(key="test_tag2", value="test_value2"),
        ]
    )
...
def test_tags_job_facet() -> None:
    ...
    tags_job_facet = tags_job.TagsJobFacet(
        tags=[
            tags_job.TagsJobFacetFields(key="test_tag", value="test_value", source="test_source"),
            tags_job.TagsJobFacetFields(key="test_tag2", value="test_value2"),
        ]
    )
...
def test_tags_dataset_facet() -> None:
    ...
    tags_dataset_facet = tags_dataset.TagsDatasetFacet(
        tags=[
            tags_dataset.TagsDatasetFacetFields(
                key="test_tag", value="test_value", field="email", source="test_source"
            ),
            tags_dataset.TagsDatasetFacetFields(key="test_tag2", value="test_value2"),
        ]
    )
    ...
    event = RunEvent(
        ...
        inputs=[
            InputDataset(
                namespace="some-namespace", name="input-dataset", inputFacets={"tags": tags_dataset_facet}
            )
        ],
        outputs=[],
    )
