...
class OpenLineageHttp implements Closeable {
    ...
    OpenLineageHttp(@NonNull final HttpClient http, @NonNull final URL baseUrl, @Nullable final String apiKey) {
        this.http = http;
    ...
    URL url(String path, Map<String, Object> queryParams) {
        try {
            final URIBuilder builder = new URIBuilder(baseUrl.toURI()).setPath(baseUrl.getPath() + BASE_PATH + path);
