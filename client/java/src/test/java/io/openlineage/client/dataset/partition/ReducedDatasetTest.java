...
class ReducedDatasetTest {
  ...
  DatasetConfig config = new DatasetConfig(Collections.emptyMap(), true, true);
  ...
  @Test
  void testTrimPathWhenDateDetectorDisabled() {
    DatasetConfig config = new DatasetConfig(Collections.emptyMap(), true, false);

    when(dataset.getName()).thenReturn("/a/b/2025-01-01");
    reducedDataset = ReducedDataset.of(config, dataset);
    assertThat(reducedDataset.getTrimmedPath()).isEqualTo("/a/b/2025-01-01");
  }

  @Test
  void testTrimPathWhenKeyValueTrimDisabled() {
    DatasetConfig config = new DatasetConfig(Collections.emptyMap(), false, true);

    when(dataset.getName()).thenReturn("/a/b/key=value");
    reducedDataset = ReducedDataset.of(config, dataset);
    assertThat(reducedDataset.getTrimmedPath()).isEqualTo("/a/b/key=value");
  }
