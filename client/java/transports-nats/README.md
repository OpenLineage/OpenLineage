...
- `user`/`password`, `token` or `credsFile` - authentication, at most one method. Optional.
- `tlsKeystorePath`, `tlsKeystorePassword`, `tlsTruststorePath`, `tlsTruststorePassword` - TLS. Optional.
- `properties` - raw jnats options (`io.nats.client.*`). Optional.
...
Tests start a JetStream-enabled NATS server: the `nats-server` binary when it is on the `PATH`, otherwise the `nats:2`
Docker image through Testcontainers. They are skipped when neither is available.
