...
CONN_URI = "snowflake://snowflake.example/db-schema?account=test_account&database=FOOD_DELIVERY&region=us-east&warehouse=snow-warehouse&secret=hideit"
CONN_URI_WITH_EXTRA_PREFIX = "snowflake://snowflake.example/db-schema?extra__snowflake__account=test_account&extra__snowflake__database=FOOD_DELIVERY&extra__snowflake__region=us-east&extra__snowflake__warehouse=snow-warehouse&extra__snowflake__secret=hideit"
CONN_URI_URIPARSED = "snowflake://snowflake.example/db-schema?account=%5B%27test_account%27%5D&database=%5B%27FOOD_DELIVERY%27%5D&region=%5B%27us-east%27%5D&warehouse=%5B%27snow-warehouse%27%5D"
...
def test_get_connection_filter_qs_params():
    ...
    assert all(param in qs_dict for param in ["warehouse", "account", "database", "region"])
...
def test_extract_create_stage(execute_query_on_hook, get_connection):
    Source(scheme="snowflake", authority="test_account", connection_url=CONN_URI_URIPARSED)
...
def test_extract_copy_from_external_stage(
    ...
    task = SnowflakeOperator(
        ...
        sql=f"COPY INTO {DB_NAME}.{DB_TABLE_NAME} FROM 'azure://mybucket/a.csv'",
        parameters={"id": 57},
    )
    ...
    assert metadata.outputs[0].name == f"{DB_NAME}.PUBLIC.{DB_TABLE_NAME}"
