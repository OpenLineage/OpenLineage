...
public abstract class KafkaTableTestBase extends AbstractTestBase {
  ...
  public static final KafkaContainer KAFKA_CONTAINER =
      new KafkaContainer(DockerImageName.parse(DockerImageVersions.KAFKA)) {
        ...
        protected void doStart() {
          ...
          .withEnv(
              "KAFKA_TRANSACTION_MAX_TIMEOUT_MS", String.valueOf(Duration.ofHours(2).toMillis()))
  ...
  public void setup() {
    ...
    scheduleTimeoutLogger(
        ...
        () -> {
          ...
          final Map<String, TopicDescription> topicDescriptions = describeExternalTopics();
  ...
  public Map<TopicPartition, OffsetAndMetadata> getConsumerOffset(String groupId) {
    ...
    try (AdminClient admin = AdminClient.create(properties)) {
      ...
      throw new IllegalStateException(
          String.format("Fail to get consumer offsets with the group id [%s].", groupId), e);
    }
  }
  ...
  private void logTopicPartitionStatus(Map<String, TopicDescription> topicDescriptions) {
    ...
    topicDescriptions.forEach(
        (topic, description) ->
            description
                .partitions()
                .forEach(tpInfo -> partitions.add(new TopicPartition(topic, tpInfo.partition()))));
