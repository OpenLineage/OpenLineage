...
import io.openlineage.flink.visitor.KafkaTopicPatternDatasetIdentifierVisitor.ConsumerUtils;
...
class KafkaTopicPatternDatasetIdentifierVisitorTest {
  ...
  @BeforeEach
  void setup() {
    when((context.getConfig().getDatasetConfig()).isResolveTopicPattern()).thenReturn(true);
  }
  ...
  void testIsDefinedWhenPatternResolvingDisabled() {
    ...
    when((context.getConfig().getDatasetConfig()).isResolveTopicPattern()).thenReturn(false);
    assertThat(visitor.isDefinedAt(dataset)).isFalse();
