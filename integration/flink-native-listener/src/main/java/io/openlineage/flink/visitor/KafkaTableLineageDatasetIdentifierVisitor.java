...
import io.openlineage.flink.util.TableLineageDatasetImplUtil;
...
/** Class to extract dataset identifier from {@link TableLineageDatasetImpl} stored on Kafka. */
@Slf4j
public class KafkaTableLineageDatasetIdentifierVisitor implements DatasetIdentifierVisitor {
  ...
  public boolean isDefinedAt(LineageDataset dataset) {
    if (!TableLineageDatasetImplUtil.isOnClasspath()) {
      return false;
    }

    if (!TableLineageDatasetImplUtil.isAssignableFrom(dataset)) {
      return false;
    }

    TableLineageDatasetImpl table = (TableLineageDatasetImpl) dataset;
