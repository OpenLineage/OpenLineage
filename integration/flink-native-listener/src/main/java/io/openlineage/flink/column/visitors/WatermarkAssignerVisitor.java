...
/**
 * Column lineage wise, watermark assigner does not influence the lineage.
 * However, we need to support it so that we can traverse the RelNode tree.
 */
public class WatermarkAssignerVisitor implements CalciteNodeVisitor {
  ...
  public List<Expression> loadExpressions(RelNode node, ExpressionContainer expressionContainer) {
    ...
    return expressionContainer
        .getOutputsOf(input)
        .stream()
        .map(e -> TransformationExpression
            .builder()
            .inputRelNode(node)
            .uuid(e.getUuid())
