...
public class OpenLineageSparkListener extends org.apache.spark.scheduler.SparkListener {
  ...
  private ArgumentParser parseConf(SparkConf conf) {
    ...
    if (url.isPresent()) {
      ...
      String namespace =
          findSparkConfigKey(conf, SPARK_CONF_NAMESPACE_KEY, ArgumentParser.DEFAULTS.getNamespace());
      String jobName = findSparkConfigKey(conf, SPARK_CONF_JOB_NAME_KEY, ArgumentParser.DEFAULTS.getJobName());
      String runId =
          findSparkConfigKey(conf, SPARK_CONF_PARENT_RUN_ID_KEY, ArgumentParser.DEFAULTS.getParentRunId());
