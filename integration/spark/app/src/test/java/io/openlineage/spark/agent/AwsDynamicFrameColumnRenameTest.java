...
/**
 * Integration test to verify that column-level lineage correctly captures column renames when using
 * AWS Glue DynamicFrames.
 *
 * <p>This test specifically validates the fix for the bug where: - Source column: 'orderid' (no
 * underscore) - Target column: 'order_id' (with underscore) - Expected CLL: 'orderid' -> 'order_id'
 * - Bug (before fix): 'order_id' -> 'order_id' (wrong source column name)
 */
...
class AwsDynamicFrameColumnRenameTest {
  ...
  void testDynamicFrameColumnRename() {
    ...
    command
        ...
        .append(" --conf ")
        .append("spark.openlineage.columnLineage.datasetLineageEnabled=true")
        .append(" --conf ")
        .append("spark.extraListeners=")
        .append(OpenLineageSparkListener.class.getName())
        .append(" --files ")
        .append(CONTAINER_LOG4J_PROPERTIES_PATH)
        .append(" /opt/spark_scripts/dynamic_frame_column_rename.py")
    ...
    assertThat(insertEvent.getInputs().get(0).getName()).contains("source_orders");

    assertThat(insertEvent.getOutputs()).hasSize(1);
    assertThat(insertEvent.getOutputs().get(0).getName()).contains("glue-column-rename-test");

    // Verify column lineage is present
    ColumnLineageDatasetFacet columnLineage =
        insertEvent.getOutputs().get(0).getFacets().getColumnLineage();
    assertThat(columnLineage).isNotNull();
    assertThat(columnLineage.getFields()).isNotNull();

    // Verify 'order_id' output column has lineage
    assertThat(columnLineage.getFields().getAdditionalProperties()).containsKey("order_id");
    ColumnLineageDatasetFacetFieldsAdditional orderIdLineage =
        columnLineage.getFields().getAdditionalProperties().get("order_id");
    assertThat(orderIdLineage.getInputFields()).isNotEmpty();

    // THE KEY ASSERTION: Verify the source column name is 'orderid' (not 'order_id')
    String sourceColumnName = orderIdLineage.getInputFields().get(0).getField();
    assertThat(sourceColumnName)
        .as("Source column name should be 'orderid' (original), not 'order_id' (transformed)")
        .isEqualTo("orderid");

    // Verify transformation type is not IDENTITY (since column was renamed)
    String transformationType =
        orderIdLineage.getInputFields().get(0).getTransformations().get(0).getSubtype();
    assertThat(transformationType)
        .as("Transformation type should not be IDENTITY for a renamed column")
        .isNotEqualTo("IDENTITY");

    log.info("✓ Column lineage correctly shows: 'orderid' -> 'order_id'");
  }
}
