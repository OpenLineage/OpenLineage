...
class JobMetricsHolderTest {
  ...
  void testPollMetricsSumByJobId() {
    ...
    underTest.cleanUp(0);
    Map<JobMetricsHolder.Metric, Number> secondPollResult = underTest.pollWriteMetrics(0);
  ...
  void testCleanUpClearsMaps() {
    ...
    assertThat(underTest.pollWriteMetrics(0).get(Metric.WRITE_RECORDS)).isEqualTo(10L);
    underTest.cleanUp(0);
    assertThat(underTest.pollWriteMetrics(0)).isEmpty();
  }
