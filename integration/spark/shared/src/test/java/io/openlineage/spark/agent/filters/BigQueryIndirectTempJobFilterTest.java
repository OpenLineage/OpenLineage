...
class BigQueryIndirectTempJobFilterTest {

  OpenLineageContext context = mock(OpenLineageContext.class);
  BigQueryIndirectTempJobFilter filter = new BigQueryIndirectTempJobFilter(context);
  ...
  void testIsDisabledForInvalidUuidSuffix() {
    when(queryExecution.optimizedPlan()).thenReturn(insert);
    when(insert.outputPath())
        .thenReturn(
            new Path(validPath.replace(".spark-bigquery-local", ".spark-bigquery-invalid")));
    assertThat(filter.isDisabled(sparkListenerEvent)).isFalse();
