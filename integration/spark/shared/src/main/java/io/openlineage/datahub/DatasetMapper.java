...
public abstract class DatasetMapper {
    ...
    public DatasetMapper(OpenLineage.Dataset dataset){
        ...
        this.template = MetadataChangeProposalWrapper.builder()
                .entityType("dataset")
    ...
    MetadataChangeProposalWrapper getSchemaMetadata() {
        ...
        SchemaMetadata schemaMetadata = new SchemaMetadata()
                .setFields(new SchemaFieldArray(dataset.getFacets().getSchema().getFields().stream().map(f -> DatasetMapperUtils.getSchemaField(f)).collect(Collectors.toList())))
