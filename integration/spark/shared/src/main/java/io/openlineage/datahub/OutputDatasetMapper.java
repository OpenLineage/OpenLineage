...
public class OutputDatasetMapper extends DatasetMapper {
  ...
  public OutputDatasetMapper(
      OpenLineage.OutputDataset dataset, List<OpenLineage.InputDataset> inputs) {
    super(dataset);
    this.inputs = inputs;
  }
  ...
  private MetadataChangeProposalWrapper getUpstreams() {
    ...
    upstreams.addAll(
        inputs.stream()
            .map(DatasetMapperUtils::getUrn)
            .map(e -> new Upstream().setDataset(e).setType(DatasetLineageType.TRANSFORMED))
