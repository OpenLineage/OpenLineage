...
public class PlanUtils3 {
  ...
  public static <D extends OpenLineage.Dataset> List<D> fromDataSourceV2Relation(
    ...
    return fromDataSourceV2Relation(datasetFactory, context, relation, null);
  }
  ...
  public static <D extends OpenLineage.Dataset> List<D> fromDataSourceV2Relation(
    ...
    OpenLineage.DatasetFacets facets = builder.build();
    CatalogUtils3.getTableProviderFacet(tableCatalog, tableProperties)
        .map(provider -> builder.put("tableProvider", provider));
    return Collections.singletonList(
        datasetFactory.getDataset(di.get().getName(), di.get().getNamespace(), builder.build()));
  }
}
