  ...
  void testApplyForHadoopFsRelationDatasetVersionFacet() {
    ...
    try (MockedStatic mocked = mockStatic(PlanUtils.class)) {
      try (MockedStatic mockedFacetUtils = mockStatic(DatasetVersionDatasetFacetUtils.class)) {
        ...
        when(PlanUtils.shouldIncludeDatasetVersionFacet(false, sparkListenerEvent))
            .thenReturn(true);
        ...
        visitor.setTriggeringEvent(sparkListenerEvent);
        ...
        assertEquals(
            ...
            ((OpenLineage.DatasetVersionDatasetFacet)
                    ds.getFacets().getAdditionalProperties().get("datasetVersion"))
                .getDatasetVersion());
      }
    }
  }
