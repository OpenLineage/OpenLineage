...
class ExpressionTraverserTest {
  ...
  void copiedTraverserRetainsParameters() {
    ExpressionTraverser t1 = aTraverser(LEAF_NODE_1, OUTPUT_EXPRESSION_ID, TransformationInfo.transformation());
    ExpressionTraverser t2 = t1.copyFor(LEAF_NODE_2);

    t1.traverse();
    t2.traverse();
    
    verify(builder).addDependency(OUTPUT_EXPRESSION_ID, EXPR_ID_1, TransformationInfo.transformation());
    verify(builder).addDependency(OUTPUT_EXPRESSION_ID, EXPR_ID_2, TransformationInfo.transformation());
  }
  ...
  void shouldTraverseTheExpressionTree() {
    ...
    verify(builder).addDependency(
        OUTPUT_EXPRESSION_ID, EXPR_ID_1, TransformationInfo.indirect(TransformationInfo.Subtypes.CONDITIONAL));
    verify(builder).addDependency(
        OUTPUT_EXPRESSION_ID, EXPR_ID_2, TransformationInfo.indirect(TransformationInfo.Subtypes.CONDITIONAL));
    verify(builder).addDependency(OUTPUT_EXPRESSION_ID, EXPR_ID_3, TransformationInfo.identity());
    verify(builder).addDependency(OUTPUT_EXPRESSION_ID, EXPR_ID_3, TransformationInfo.transformation());
  }
  ...
  void handlesMaskingExpressions() {
    ...
    verify(builder).addDependency(OUTPUT_EXPRESSION_ID, EXPR_ID_1, TransformationInfo.transformation(true));
  }
  ...
  ExpressionTraverser aTraverser(Expression expression, ExprId outputExpressionId, TransformationInfo transformationInfo) {
