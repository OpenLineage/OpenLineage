...
import io.openlineage.spark.extension.v1.OutputDatasetWithDelegate;
import io.openlineage.spark.extension.v1.OutputDatasetWithFacets;
import io.openlineage.spark.extension.v1.OutputDatasetWithIdentifier;
import io.openlineage.spark.extension.v1.OutputLineageNode;
...
@Slf4j
public class SparkExtensionJavaV1OutputDatasetBuilder
    extends AbstractQueryPlanOutputDatasetBuilder<LogicalPlan> {

  public SparkExtensionJavaV1OutputDatasetBuilder(OpenLineageContext context) {
    super(context, false);
  }
  ...
  protected List<OutputDataset> apply(SparkListenerEvent event, LogicalPlan x) {
    ...
    List<OutputDatasetWithFacets> datasets =
        lineageNode.getOutputs(ExtensionPlanUtils.javaContext(event, context));
    ...
    List<OutputDataset> outputDatasets =
        datasets.stream()
            ...
            .map(outputDelegate -> (LogicalPlan) (outputDelegate.getNode()))
  ...
  protected List<OutputDataset> delegate(SparkListenerEvent event, LogicalPlan plan) {
    return delegate(
        ...
        .applyOrElse(
            plan, ScalaConversionUtils.toScalaFn((lp) -> Collections.<OutputDataset>emptyList()))
        .stream()
        .collect(Collectors.toList());
  }
