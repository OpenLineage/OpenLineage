...
/**
 * Extracts the modified table of a Databricks {@code DELETE FROM} as an output dataset.
 *
 * <p>{@link TableContentChangeDatasetBuilder} covers the catalyst {@code DeleteFromTable} node,
 * which Databricks never produces - the runtime resolves the statement to a Delta command instead.
 ...
 */
@Slf4j
public class DeleteCommandOutputDatasetBuilder
    extends AbstractQueryPlanOutputDatasetBuilder<LogicalPlan> {

  public DeleteCommandOutputDatasetBuilder(OpenLineageContext context) {
    super(context, false);
  }

  @Override
  public boolean isDefinedAtLogicalPlan(LogicalPlan x) {
    return DeleteCommandUtils.isDeleteCommand(x);
  }

  @Override
  protected List<OutputDataset> apply(SparkListenerEvent event, LogicalPlan x) {
    return DeleteCommandUtils.target(x)
  ...
  public Optional<String> jobNameSuffix(LogicalPlan plan) {
    return DeleteCommandUtils.target(plan)
        .flatMap(
            target ->
