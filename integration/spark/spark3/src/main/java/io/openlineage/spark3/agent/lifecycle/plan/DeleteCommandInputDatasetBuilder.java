...
/**
 * Extracts input datasets from the subqueries of a {@link DeleteFromTable} or of the Databricks
 * Delta command the same statement is resolved to.
 *
 * <p>Tables referenced from a WHERE clause live inside expressions, not inside the children of the
 * plan node:
 *
 * <pre>
 * DeleteFromTable
 ...
 * their expressions.
 */
public class DeleteCommandInputDatasetBuilder
    extends AbstractQueryPlanInputDatasetBuilder<LogicalPlan> {

  public DeleteCommandInputDatasetBuilder(OpenLineageContext context) {
    super(context, false);
  }

  @Override
  public boolean isDefinedAtLogicalPlan(LogicalPlan x) {
    return (x instanceof DeleteFromTable) || DeleteCommandUtils.isDeleteCommand(x);
  }
