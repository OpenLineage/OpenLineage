...
public class FlinkCassandraApplication {
    ...
    public static void main(String[] args) throws Exception {
        ...
        CassandraPojoInputFormat<Event> eventInputFormat = new CassandraPojoInputFormat<>(
                query,
                createClusterBuilder(),
                Event.class,
                () -> new Mapper.Option[]{});

        CassandraSink sink = CassandraSink.addSink(env.createInput(eventInputFormat, TypeInformation.of(Event.class)).uid("cassandra-source"))
    ...
    private static ClusterBuilder createClusterBuilder() {
        ...
        return new ClusterBuilder() {
            ...
            protected Cluster buildCluster(Cluster.Builder builder) {
                return builder.addContactPointsWithPorts(new InetSocketAddress(properties.getProperty(CASSANDRA_HOST_NAME), Integer.parseInt(properties.getProperty(CASSANDRA_POST))))
                        .withQueryOptions(
                                new QueryOptions()
                        ...
                        .withSocketOptions(
                                new SocketOptions()
                                        .setConnectTimeoutMillis(Integer.parseInt(properties.getProperty(CASSANDRA_CONNECT_TIMEOUT)))
                                        .setReadTimeoutMillis(Integer.parseInt((properties.getProperty(CASSANDRA_READ_TIMEOUT))))
                        ).build();
            }
        };
    }
