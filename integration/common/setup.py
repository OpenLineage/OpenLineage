...
extras_require["dev"] = list(set(sum(extras_require.values(), [])))
extras_require["dev_no_parser"] = list(
    set(
        sum(
            {k: extras_require[k] for k in extras_require.keys() if k not in ["sql", "dev"]}.values(),
            [],
        )
    )
)
