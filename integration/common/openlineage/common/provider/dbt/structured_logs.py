...
class DbtStructuredLogsProcessor(DbtLocalArtifactProcessor):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.dbt_log_dirname: str = os.path.join(os.path.split(self.dbt_log_file_path)[:-1])
    ...
    def parse_node_finished_event(self, event) -> RunEvent:
        ...
        if node_status == "skipped":
            ...
            self.logger.info("node {} has an unknown node status {}", node_unique_id, node_status)
    ...
    def _run_dbt_command(self) -> Generator[str, None, None]:
        ...
        self.logger.debug("Opened log file `{}`", self.dbt_log_file_path)
        ...
        try:
            while process.poll() is None:
                ...
                if current_size > last_size:
                    self.logger.debug(
                        "Current size: {}, last size: {}, to read: {}",
                        current_size,
                        last_size,
