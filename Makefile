...
setup-all: ## 🚀 Setup all integrations
	@echo "$(BLUE)Setting up all integrations...$(NC)"
	@$(MAKE) setup-client
	@$(MAKE) setup-common
	@$(MAKE) setup-airflow
	@$(MAKE) setup-dagster
	@$(MAKE) setup-dbt
	@echo "$(GREEN)✅ All integrations set up successfully!$(NC)"

setup-client: ## Setup Python client
	@echo "$(BLUE)Setting up Python client...$(NC)"
	cd client/python && uv sync --extra tests

setup-common: ## Setup integration common library
	@echo "$(BLUE)Setting up integration common...$(NC)"
	cd integration/common && uv sync --extra tests

setup-airflow: ## Setup Airflow integration (with airflow-compatible deps)
	@echo "$(BLUE)Setting up Airflow integration...$(NC)"
	cd integration/airflow && uv sync --extra airflow --extra tests

setup-dagster: ## Setup Dagster integration (with dagster-compatible deps)
	@echo "$(BLUE)Setting up Dagster integration...$(NC)"
	cd integration/dagster && uv sync --extra tests

setup-dbt: ## Setup dbt integration
	@echo "$(BLUE)Setting up dbt integration...$(NC)"
	cd integration/dbt && uv sync --extra tests
...
test-airflow: ## Test Airflow integration
	...
	cd integration/airflow && uv run pytest tests/

test-dagster: ## Test Dagster integration
	@echo "$(BLUE)Testing Dagster integration...$(NC)"
	cd integration/dagster && uv run pytest tests/
...
lint-types: ## Run mypy type checking per integration
	...
	cd integration/dagster && uv run mypy src/ --ignore-missing-imports
...
dagster: ## Enter Dagster integration directory
	@echo "$(GREEN)⚡ Switching to Dagster integration$(NC)"
	@echo "Run: cd integration/dagster && uv sync --extra tests"
	@cd integration/dagster && bash
...
status: ## Show status of all integrations
	...
	@echo -n "Dagster: "; \
	if [ -d "integration/dagster/.venv" ]; then echo "$(GREEN)✅ Ready$(NC)"; else echo "$(RED)❌ Not setup$(NC)"; fi
	@echo -n "dbt: "; \
...
deps: ## Show dependency versions (for debugging conflicts)
	...
	@echo "$(YELLOW)Dagster protobuf:$(NC)"
	@cd integration/dagster && uv run python -c "import google.protobuf; print(f'  protobuf: {google.protobuf.__version__}')" 2>/dev/null || echo "  Not installed"
...
clean: ## Clean all virtual environments and caches
	...
	rm -rf integration/dagster/.venv
