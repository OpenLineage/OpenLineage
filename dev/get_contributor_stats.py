...
class ContributorStats:
    ...
    def sort_key(self, row):
        ...
        sort_map = {
            ...
            "PRs": 9,
            "all-time-commits": 5,
            "all-time-additions": 3,
            "additions-this-period": 6,
            "deletions-this-period": 7,
            "all-time-deletions": 4,
        }
        index = sort_map.get(self.sort, None)
        if index:
            return row[index]
        return None
    ...
    def collect_committers(self):
        ...
        if len(self.committer_changes) > 0:
            self.committer_avg = round(sum(self.committer_changes) / len(self.committer_changes), 2)
        else:
        ...
        for row in self.rows:
            if row[8] > 0:
                ...
                if self.committer_avg != "N/A":
                    avg_diff = round(((row[6] + row[7]) / row[8]) - self.committer_avg, 2)
                row.append(avg)
                row.append(avg_diff)
...
@click.option(
    ...
    default="openlineage/OpenLineage",
)
@click.option(
    "--org",
    help="Search org",
