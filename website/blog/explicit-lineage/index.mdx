...
OpenLineage expresses lineage through `inputs` and `outputs` arrays on run and job events. The implicit rule is straightforward: everything in `inputs` feeds everything in `outputs`. For the common case - one job, one input, one output - this works perfectly. Most Spark jobs, dbt models, and Airflow tasks produce a single logical output, so all recorded inputs can be associated with that output without ambiguity, and correct data lineage is captured. 

The limitation however becomes more noticeable when a single job handles several independent data flows. Consider a bulk ETL job that reads tables A and B and writes tables C and D, where A feeds C and B feeds D. From the event alone, a consumer may infer four edges: A → C, A → D, B → C, and B → D. The additional edges do not reflect the intended flow and can make the resulting lineage graph less precise.
...
The proposal introduces two new facets - `LineageJobFacet` and `LineageDatasetFacet` - that allow producers to describe lineage relationships directly. Instead of assuming that every input contributes to every output, consumers can use the relationships provided by the producer. The facets support dataset-level and column-level mappings, dataset-to-dataset derivations, and data passed directly between jobs, enabling OpenLineage producers to capture lineage on any level of granularity granularity, and describe also relationships that do not fit the input-job-output conceptualization.
...
The `inputs` and `outputs` arrays remain populated for backward compatibility and for dataset facet attachments. But when a lineage facet is present, consumers should derive edges from it - not from the cartesian product.
...
Sunsetting of ColumnLineage facet by the new facets will be gradual. To make adoption easier for teams at different stages of migration, the proposal extends the OpenLineage Java and Python clients with translation between the two representations during event emission. 

A client can derive new lineage facets from existing `inputs`, `outputs`, and CLL. Other way around, the client can generate the legacy representation from explicit lineage facets (but will lose information about lineage the legace model cannot express today - such as dataset-only or job-to-job lineage).

Translation applies to `RunEvent` and `JobEvent`; `DatasetEvent` lineage has no legacy equivalent. A configuration option controls which translations the client performs:
...
If you maintain an OpenLineage integration, this proposal gives you a path to express more information about your data flows, not just what fits the current model. The new facets let producers represent lineage at the level of detail available to them: precise dataset-level edges where known, column-level mappings where available, and mixed levels of detail within the same event.
...
*The Explicit Lineage Facet is an experimental feature under active review. Feedback is welcome via the GitHub issue. The proposal may be changed or updated based on community input.*
