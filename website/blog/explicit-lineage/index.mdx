...
A client can derive new lineage facets from existing `inputs`, `outputs`, and CLL. Other way around, the client can generate the legacy representation from explicit lineage facets (but will lose information about lineage the legacy model cannot express today - such as dataset-only or job-to-job lineage).
