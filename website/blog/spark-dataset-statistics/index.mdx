...
OpenLineage events are made of three main building blocks: runs, jobs, and datasets. Unfortunately,
metadata about data processed is both run and dataset property.
...
spark = (SparkSession.builder.master('local')
         ...
         .config('spark.jars', '/home/jovyan/openlineage/libs/shadow/openlineage-spark-agent_2.12-1.26.0-SNAPSHOT-shadow.jar')
...
By default, a `LoggingMetricsReporter` is configured, but the API allows to implement custom reporters
[see link](https://iceberg.apache.org/docs/latest/metrics-reporting/#via-catalog-configuration).
This what OpenLineage Spark agent does. It instantiates `OpenLineageMetricsReporter` and injects
it into Iceberg catalog. Statistics sent into `OpenLineageMetricsReporter` are collected and
...
{
  "_producer": "https://github.com/OpenLineage/OpenLineage/tree/1.26.0-SNAPSHOT/integration/spark",
  "_schemaURL": "https://openlineage.io/spec/facets/1-0-0/IcebergCommitReportInputDatasetFacet.json",
  "operation": "append",
  "commitMetrics": {
    "totalDuration": 64,
    "attempts": 1,
...
In this post we introduced the recent developments in Spark dataset statistics collection and reporting within OpenLineage events.
We outlined the basic statistics included, as well as the detailed scan and commit reports generated when processing Iceberg datasets.
These enhancements provide a more detailed view of the data processed by Spark jobs, enabling users to better understand and monitor their data pipelines.
