...
| openlineage.flink.checkpointTrackingRestUrl         | Overrides the Flink JobManager REST API base URL used by the checkpoint tracker. Set this when the effective REST port bound at runtime differs from the configured `rest.port` (for example when `rest.port`/`rest.bind-port` is `0`, a range, or already in use), such as in Kubernetes Application mode deployments.  | `http://localhost:44193` |
