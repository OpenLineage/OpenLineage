...
| openlineage.restApiBaseUrl                          | Overrides the Flink REST API base URL used by checkpoint tracking when the effective runtime endpoint differs from the configured `rest.address` and `rest.port`, for example in Application mode deployments.        | `http://host:8081`      |
